...
void RTC_SetCounter_(uint32_t count)                                                    //Çàïèñàòü íîâîå çíà÷åíèå ñ÷åò÷èêà
{
  RCC->APB1ENR |= RCC_APB1ENR_PWREN | RCC_APB1ENR_BKPEN;            //âêëþ÷èòü òàêòèðîâàíèå PWR è Backup
  PWR->CR |= PWR_CR_DBP;                                            //ðàçðåøèòü äîñòóï ê Backup îáëàñòè
  while (!(RTC->CRL & RTC_CRL_RTOFF));                              //ïðîâåðèòü çàêîí÷åíû ëè èçìåíåíèÿ ðåãèñòðîâ RTC
  RTC->CRL |= RTC_CRL_CNF;                                          //Ðàçðåøèòü Çàïèñü â ðåãèñòðû RTC
  RTC->CNTH = count>>16;                                                              //çàïèñàòü íîâîå çíà÷åíèå ñ÷åòíîãî ðåãèñòðà
  RTC->CNTL = count;
  RTC->CRL &= ~RTC_CRL_CNF;                                                       //Çàïðåòèòü çàïèñü â ðåãèñòðû RTC
  while (!(RTC->CRL & RTC_CRL_RTOFF));                                         //Äîæäàòüñÿ îêîí÷àíèÿ çàïèñè
  PWR->CR &= ~PWR_CR_DBP;                                                         //çàïðåòèòü äîñòóï ê Backup îáëàñòè
}

uint32 RTC_GetCounter_(void)                                                             //Ïîëó÷èòü çíà÷åíèå ñ÷åò÷èêà
{
